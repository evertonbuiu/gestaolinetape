-- Corrigir a função de atualização de estoque de equipamentos
-- para incluir todos os status que representam equipamentos em uso
...
BEGIN
    ...
    IF equipment_id_to_update IS NOT NULL THEN
        ...
        RAISE NOTICE 'Updated equipment % - Total rented: %, Returned: %, Maintenance: %, Net rented: %, Available: %', 
                     equipment_name_to_update, total_rented, total_returned, total_maintenance, net_rented,
                     (total_stock - net_rented - total_maintenance);
    ELSE
...
BEGIN
    ...
    FOR eq_record IN SELECT * FROM public.equipment LOOP
        ...
        RAISE NOTICE 'Recalculated equipment %: Total=%, Rented=%, Returned=%, Maintenance=%, Available=%', 
                     eq_record.name, eq_record.total_stock, total_rented, total_returned, total_maintenance,
                     (eq_record.total_stock - net_rented - total_maintenance);
    END LOOP;
        
    RAISE NOTICE 'Equipment stock recalculated for all items';
END;
$function$;